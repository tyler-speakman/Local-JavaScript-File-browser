define(["infrastructure/AjaxQueueServices"], function (ajaxQueueServices) {
    ...
    function getStructure(path, parent, depth) {
        ...
        function handleUrlResponse(data, textStatus, jqXHR) {
            return getFsEntitiesFromChromeResponseText(data);
        }
        ...
        function handleUrlResponseSuccess(fsEntities) {
            ...
            while (index--) {
                var fsEntity = fsEntities[index];
                parent.children.push(fsEntity);
                fsEntity.parent = parent;
                var fsEntityFullPath = fsEntity.path + ((fsEntity.isFolder) ? "\\" : "");
                var subRequestDepth = depth - 1;
                if (fsEntity.isFolder && subRequestDepth >= 0 && fsEntity.name !== ".." && fsEntity.name !== "") {
        ...
        function handleUrlResponseFailure(jqXHR, textStatus) {
            ...
            handleFsTreeResponseFailure(jqXHR, textStatus, path);
        }

        //#endregion
    }

    function handleFsTreeResponseFailure(jqXHR, textStatus, requestUrl) {
        if (jqXHR.status === 404) {
        } else {
    ...
    function getFsEntitiesFromChromeResponseText(fsRequestText) {
        ...
        while (fileMatch = fileRegExp.exec(fsRequestText)) {
            ...
            var isFolder = fileMatch[3] === "1" || isUpDir;
            var isFile = !isFolder && !isUpDir;

            var path = (parentPath + fileMatch[1]).replace(/\\\\/gi, "\\").replace(/\\/gi, "/");;

            var fsEntity;
            if (isFile) {
                fsEntity = new FsEntity({ name: fileMatch[1], path: path, isFolder: isFolder, isFile: true, isAccessible: true, isUpDir: false, size: fileMatch[4], lastModifiedDate: new Date(fileMatch[5]), });
            } else if (isFolder && !isUpDir) {
                fsEntity = new FsEntity({ name: fileMatch[1], path: path, isFolder: true, isFile: false, isAccessible: true, isUpDir: false, size: NaN, children: [] });
            } else if (isUpDir) {
                fsEntity = new FsEntity({ name: "..", path: path, isFolder: true, isFile: false, isAccessible: true, isUpDir: true, size: NaN });
            }
    ...
    function InaccessibleFsEntity() {
        return new FsEntity({ path: "NA", name: "NA", encodedName: "NA", isFolder: false, isFile: false, isUpdir: false, isAccessible: false, size: NaN });
    }
    ...
    function FsEntity(o) {
        ...
        self.isFolder = o.isFolder;
        self.isUpDir = o.isUpDir;
        self.isAccessible = o.isAccessible;
        self.isLoaded = false;

        self.lastModifiedDate = o.lastModifiedDate;
        ...
        if (self.isFile) {
            self.extension = self.name.replace(/^.+\.([^\.]+)$/gi, "$1");
            self.type = getMimeTypeByExtension(self.extension);
        }

        self.hierarchy = function () { return getFsHierarchy(self); };

        return self;

        //#region Internal Methods

        function getFsHierarchy(fsEntity) {
            // Builds an an array of the entities ancestry (i.e., entity's parent, entity's parent's parent, entity's parent's parent's parent, etc.)
            var ancestry = [fsEntity];
            while (ancestry[0].parent) {
                ancestry.unshift(ancestry[0].parent);
            }

            // Add flags for the first and last entities
            var length = ancestry.length;
            var index = length;
            while (index--) {
                var item = ancestry[index];
                item.isFirst = (item.isFirst || ko.observable());
                item.isLast = (item.isLast || ko.observable());
